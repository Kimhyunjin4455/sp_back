...
public class ProductListRepositoryCustomImpl implements ProductListRepositoryCustom {
	...
	//
	// 	// 카테고리명으로 카테고리 ID 조회
	// 	Long majorCategoryId = jpaQueryFactory
	// 		.select(categoryList.id)
	// 		.from(categoryList)
	// 		.where(categoryList.categoryName.eq(categoryName))
	// 		.fetchOne();
	//
	// 	Long middleCategoryId = jpaQueryFactory
	// 		.select(categoryList.id)
	// 		.from(categoryList)
	// 		.where(categoryList.categoryName.eq(middleCategoryName))
	// 		.fetchOne();
	//
	// 	// 마지막 ID 커서 적용
	// 	Optional.ofNullable(lastId)
	...
	//
	// 	// productCode 리스트로 변환
	// 	List<String> productUuidList = content.stream()
	// 		.map(Product::getProductUuid)
	// 		.toList();
	//
	// 	// CursorPage 객체 반환
	// 	return new CursorPage<>(productUuidList, nextCursor, hasNext, currentPageSize, currentPage);
	// }
